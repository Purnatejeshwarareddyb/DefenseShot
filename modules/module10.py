...
def open_module_11():
    ...
    possible_paths = [
        r"C:\Users\B.PURNA\PycharmProjects\DefenseShot\modules\module11.py",  # Original path
        os.path.join(os.path.dirname(__file__), "modules", "module11.py"),  # Relative to current file
        os.path.join(os.path.dirname(__file__), "module11.py"),  # Same directory
        "modules/module11.py",  # Relative path
        "module11.py"  # Current directory
    ]
...
questions = [
{
"question": "The IAF Compendium of Challenges & Opportunities for Indian Industry is described as an 'invaluable resource' that encapsulates strategic challenges and what else at the forefront of National defense and Air superiority? ",


"options": ["A. Financial burdens", "B. Technological opportunities", "C. Historical precedents", "D. Personnel limitations"],
"answer": "B",
"difficulty": "Easy",
"category": "Compendium Purpose"
},
{
"question": "What is one of the key government schemes propelling the journey towards innovative and indigenous product realization, as mentioned in the Defence Minister's message? ",


"options": ["A. Digital India", "B. iDEX", "C. Swachh Bharat Abhiyan", "D. Smart Cities Mission"],
"answer": "B",
"difficulty": "Easy",
"category": "Government Initiatives"
},
{
"question": "According to the Chief of the Air Staff, IAF is giving impetus towards self-reliance and what other national initiative? ",


"options": ["A. Skill India", "B. Make in India", "C. Clean India", "D. Fit India"],
"answer": "B",
"difficulty": "Medium",
"category": "National Vision"
},
{
"question": "The compendium aims to bring 'problem statements' to the table for industry partners to do what, based on their capabilities? ",


"options": ["A. Critique them", "B. Pick their challenges", "C. Ignore them", "D. Forward them"],
"answer": "B",
"difficulty": "Medium",
"category": "Industry Engagement"
},
{
"question": "Which of the following is listed as an ongoing project under the 'Weapon System' category in Section-I? ",


"options": ["A. Advanced Radar Systems", "B. 125 Kg Bomb", "C. Cyber Security Protocols", "D. Space Debris Removal"],
"answer": "B",
"difficulty": "Medium",
"category": "Ongoing Projects"
},
{
"question": "Section-II of the compendium comprises projects open for participation by which entities? ",


"options": ["A. Only large Industries", "B. Start-Ups / MSMEs / large Industries", "C. Only government organizations", "D. International corporations"],
"answer": "B",
"difficulty": "Hard",
"category": "Project Participation"
},
{
"question": "What is one of the futuristic capabilities mentioned in the Defence Minister's message that the compendium forms the bedrock of concepts towards realizing? ",


"options": ["A. Underwater Drones", "B. Next Gen Fighter Aircraft", "C. High-Speed Rail Systems", "D. Civilian Satellite Communication"],
"answer": "B",
"difficulty": "Hard",
"category": "Futuristic Capabilities"
},
{
"question": "What is the focused aim of the Directorate of Aerospace Design (DAD) at Air HQ? ",


"options": ["A. Managing logistics and supply chain", "B. Facilitating innovations in IAF and increased interaction with private industries", "C. Overseeing personnel training", "D. Developing international military alliances"],
"answer": "B",
"difficulty": "Medium",
"category": "Organizational Goals"
},
{
"question": "The IAF's push towards operational preparedness is a reflection of the geo-political situation in various theatres of the world, as stated by whom? ",


"options": ["A. Deputy Chief of the Air Staff", "B. Chief of the Air Staff", "C. Defence Minister", "D. Director of Aerospace Design"],
"answer": "B",
"difficulty": "Medium",
"category": "Strategic Context"
},
{
"question": "What does the compendium primarily aim to bridge the gap between? ",


"options": ["A. Past achievements and future plans", "B. The Indian Air Force's needs and the industry's capabilities", "C. Military and civilian technologies", "D. Research and development phases"],
"answer": "B",
"difficulty": "Medium",
"category": "Compendium Purpose"
}
...
class DefenseTrainingSystem:
    def __init__(self):
        # Initialize all DSA-based components
        self.hash_table = {}  # For user data
        self.binary_tree = BinaryTree()  # For decision making
        self.graph = Graph()  # For route planning
        self.sorting_algo = QuickSort()  # For intelligence analysis
        self.dp_solver = DynamicProgramming()  # For resource optimization
    ...
    def plan_route(self, points):
        ...
        return self.graph.find_shortest_path(points)

    def analyze_data(self, data):
        # Using sorting algorithms
        sorted_data = self.sorting_algo.sort(data)
        return self.binary_tree.analyze(sorted_data)

    def optimize_resources(self, requirements):
        # Using dynamic programming
        return self.dp_solver.solve(requirements)


class BinaryTree:
    """Binary tree implementation for decision making"""
    ...
    def search(self, value):
        # BST search with visualization steps
        pass


class Graph:
    """Graph implementation for route planning"""
...
class QuickSort:
    """Sorting implementation for intelligence analysis"""
...
class DynamicProgramming:
    """DP solver for resource optimization"""

    def solve(self, requirements):
...
def draw_menu(surface):
    ...
    subtitle_text = big_font.render("Defense Training Module 10", True, WHITE)
    ...
    options = [
        ...
        "2. Take Quiz",
        "3. Practice Range",
        "4. DSA Visualizations",  # New option
        "5. Exit"
    ]
...
def pdf_reader():
    ...
    try:
        pdf_path = r"C:\Users\B.PURNA\PycharmProjects\DefenseShot\study_materials\module_10.pdf"  # Make sure this file exists
...
def dsa_visualizations():
    """Interactive DSA concept visualizations"""
    ...
    while True:
        ...
        if animation_running:
            animation_step += 1
            if animation_step > 180:  # 3 seconds at 60fps
                animation_running = False
                animation_step = 0
        ...
        # Title
        title_text = title_font.render("DSA Visualizations", True, YELLOW)
        screen.blit(title_text, (WIDTH // 2 - title_text.get_width() // 2, 50))

        # Draw options
        for i, option in enumerate(visualization_options):
            color = YELLOW if i == current_option else WHITE
        ...
        if animation_running:
            draw_dsa_visualization(screen, current_option, animation_step, particles)
            particles.update()

        # Instructions
        instructions = font.render("Select a visualization (1-6), ESC to return", True, WHITE)
...
def draw_dsa_visualization(surface, option, step, particles):
    """Draw animations for different DSA concepts in the right corner"""
    # Changed positioning to right corner
    x_center = WIDTH - 250  # Right side with some margin
    y_center = HEIGHT // 2 - 100  # Upper portion of right side

    # Create a background area for the visualization
    viz_rect = pygame.Rect(WIDTH - 350, 150, 330, 300)
    pygame.draw.rect(surface, (20, 20, 40), viz_rect)  # Dark background
    pygame.draw.rect(surface, WHITE, viz_rect, 2)  # White border

    if option == 0:  # Hash Tables
        # Draw hash table animation in right corner
        title = font_small.render("Hash Table: Key-Value Storage", True, CYAN)
        surface.blit(title, (x_center - title.get_width() // 2, 160))

        # Draw hash table structure (smaller size for corner)
        table_rect = pygame.Rect(x_center - 120, y_center - 50, 240, 100)
        pygame.draw.rect(surface, WHITE, table_rect, 2)

        # Draw vertical lines for buckets
        for i in range(5):
            pygame.draw.line(surface, WHITE,
                             (x_center - 120 + i * 48, y_center - 50),
                             (x_center - 120 + i * 48, y_center + 50), 1)

        # Animate insertion
        if step < 60:
            key = "user_" + str(step // 10)
            value = "data_" + str(step // 10)
            hash_pos = x_center - 96 + ((step // 10) % 5) * 48

            # Draw key-value pair moving to position
            pygame.draw.circle(surface, GREEN, (x_center, y_center - 80 + step // 2), 8)
            key_text = font_small.render(key, True, BLACK)
            surface.blit(key_text, (x_center - key_text.get_width() // 2, y_center - 80 + step // 2 - 5))

            if step > 30:
                # Show hashing to bucket
                pygame.draw.line(surface, YELLOW, (x_center, y_center - 80 + 15),
                                 (hash_pos, y_center - 45), 2)

        # Show collision resolution
        elif step < 120:
            # Draw linked list for collision
            hash_pos = x_center - 96 + 96
            pygame.draw.line(surface, WHITE, (hash_pos, y_center - 25), (hash_pos + 25, y_center - 15), 2)
            pygame.draw.line(surface, WHITE, (hash_pos + 25, y_center - 15), (hash_pos + 15, y_center + 5), 2)

            # Add particles for visualization
            if step % 10 == 0:
                particles.add_explosion(hash_pos + 20, y_center, BLUE)

    elif option == 1:  # Trees & Graphs
        # Draw tree animation in right corner
        title = font_small.render("Binary Search Tree Operations", True, CYAN)
        surface.blit(title, (x_center - title.get_width() // 2, 160))

        # Draw tree structure (smaller size)
        root_pos = (x_center, y_center - 20)
        if step > 0:
            pygame.draw.circle(surface, WHITE, root_pos, 15)
            val_text = font_small.render("50", True, BLACK)
            surface.blit(val_text, (root_pos[0] - 8, root_pos[1] - 8))

        if step > 30:
            left_pos = (x_center - 35, y_center + 20)
            pygame.draw.line(surface, WHITE, root_pos, left_pos, 2)
            pygame.draw.circle(surface, WHITE, left_pos, 15)
            val_text = font_small.render("30", True, BLACK)
            surface.blit(val_text, (left_pos[0] - 8, left_pos[1] - 8))

        if step > 60:
            right_pos = (x_center + 35, y_center + 20)
            pygame.draw.line(surface, WHITE, root_pos, right_pos, 2)
            pygame.draw.circle(surface, WHITE, right_pos, 15)
            val_text = font_small.render("70", True, BLACK)
            surface.blit(val_text, (right_pos[0] - 8, right_pos[1] - 8))

        if step > 90:
            # Show search path
            search_val = 70 if step < 150 else 25
            path_color = GREEN if step < 150 else RED

            if step < 150:
                pygame.draw.line(surface, path_color, root_pos, right_pos, 3)
            else:
                pygame.draw.line(surface, path_color, root_pos, left_pos, 3)

    elif option == 2:  # Sorting Algorithms
        # Draw sorting animation in right corner
        title = font_small.render("Quick Sort Algorithm", True, CYAN)
        surface.blit(title, (x_center - title.get_width() // 2, 160))
        ...
        for i, val in enumerate(arr):
            x = x_center - 100 + i * bar_width
            height = (val / 90) * max_height
            y = y_center + 50 - height

            # Color based on step for animation
            if step < 60:
                color = RED if i == step // 10 else WHITE
            elif step < 120:
                color = GREEN if i <= step // 10 else WHITE
            else:
                color = BLUE
            ...
            # Draw value
            val_text = font_small.render(str(val), True, WHITE)
            surface.blit(val_text, (x + 5, y_center + 60))

    elif option == 3:  # Graph Algorithms
        # Draw graph algorithm animation in right corner
        title = font_small.render("Graph Traversal (BFS)", True, CYAN)
        surface.blit(title, (x_center - title.get_width() // 2, 160))
        ...
        for i, node in enumerate(nodes):
            if step > i * 20:
                color = GREEN if step > i * 20 + 10 else YELLOW
            else:
                color = WHITE
            pygame.draw.circle(surface, color, node, 12)
            pygame.draw.circle(surface, BLACK, node, 12, 2)

            # Node label
            label = font_small.render(str(i), True, BLACK)
            surface.blit(label, (node[0] - 5, node[1] - 5))

    elif option == 4:  # Dynamic Programming
        # Draw DP animation in right corner
        title = font_small.render("Dynamic Programming", True, CYAN)
        surface.blit(title, (x_center - title.get_width() // 2, 160))
        ...
        for i, val in enumerate(fib_values):
            if step > i * 15:
                ...
                val_text = font_small.render(str(val), True, BLACK)
                surface.blit(val_text, (x - 5, y - 5))

                # Draw connection lines
                if i > 0:
                    prev_x = x_center - 100 + (i - 1) * 25
                    pygame.draw.line(surface, GREEN, (prev_x + 12, y), (x - 12, y), 2)

                # Show calculation
                if i >= 2 and step > i * 15 + 10:
                    calc_text = font_small.render(f"F({i}) = F({i - 1}) + F({i - 2})", True, WHITE)
                    surface.blit(calc_text, (x_center - 80, y_center + 40))

    # Draw particles in the right corner area
        ...
        if current_state == "menu":
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    elif event.key == pygame.K_4:  # New DSA visualization option
                        current_state = "dsa"
                    elif event.key == pygame.K_5:
                        pygame.quit()
                        sys.exit()
