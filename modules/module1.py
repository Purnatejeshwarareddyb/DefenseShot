...
def open_module_2():
    ...
    possible_paths = [
        r"C:\Users\B.PURNA\PycharmProjects\DefenseShot\modules\module2.py",  # Original path
        os.path.join(os.path.dirname(__file__), "modules", "module2.py"),  # Relative to current file
        os.path.join(os.path.dirname(__file__), "module2.py"),  # Same directory
        "modules/module2.py",  # Relative path
        "module2.py"  # Current directory
    ]
...
try:
    background_image = pygame.image.load(R"C:\Users\B.PURNA\PycharmProjects\DefenseShot\assets\images\background1.jpg")
...
questions = [
    {
        "question": "What is the fundamental objective of India's defence policy as per the Ministry of Defence report?",
        "options": [
            "A. To ensure peace and development",
            "B. To expand territorial influence",
            "C. To dominate neighboring countries",
            "D. To become a global military power"
        ],
        "answer": "A",
        "difficulty": "Easy",
        "category": "Policy"
    ...
    {
        "question": "Who is responsible for coordinating all four departments in the Ministry of Defence?",
        "options": [
            "A. Defence Secretary",
    ...
    {
        "question": "What is the main role of the Indian Army?",
        "options": [
            "A. Safeguard territorial integrity against external aggression",
            "B. Conduct naval patrols",
            "C. Manage defence procurements",
    ...
    {
        "question": "Which of the following is a focus area in Indian Army’s modernization plan?",
        "options": [
            "A. Improvement in fire power and mobility",
            "B. Acquisition of luxury helicopters",
            "C. Building new naval ports",
            "D. Launching satellites for education"
        ],
        "answer": "A",
        "difficulty": "Medium",
    ...
    {
        "question": "What is the role of the Territorial Army?",
        "options": [
            "A. Support civil authorities during emergencies",
            "B. Participate in international maritime security",
            "C. Engage in airspace surveillance",
            "D. Oversee defence budget allocations"
        ],
        "answer": "A",
        "difficulty": "Medium",
    ...
    {
        "question": "What is India's approach towards the situation in Sri Lanka?",
        "options": [
            "A. Support a united Sri Lanka addressing all communities",
            "B. Military intervention in conflict zones",
            "C. Economic sanctions",
            "D. Direct peace enforcement"
        ],
        "answer": "A",
        "difficulty": "Medium",
    ...
    {
        "question": "What significant peacekeeping contribution has the Indian Army made?",
        "options": [
            "A. Participated in 43 UN missions with over 90,000 troops",
            "B. Formed a regional peacekeeping council",
            "C. Deployed drones in conflict zones",
            "D. Trained African military leaders"
        ],
        "answer": "A",
        "difficulty": "Hard",
    ...
    {
        "question": "What is the Army’s strategy in counterinsurgency operations in Jammu & Kashmir?",
        "options": [
            "A. People-centric operations with minimal public inconvenience",
            "B. Imposing curfews",
            "C. Large-scale military presence in civilian areas",
            "D. Blocking communication lines"
        ],
        "answer": "A",
        "difficulty": "Hard",
    ...
    {
        "question": "What system has been introduced to improve Indian Army's communication infrastructure?",
        "options": [
            "A. Optical Fibre Cable network in alliance with BSNL",
            "B. Submarine communication lines",
            "C. Satellite-only networks",
    ...
    {
        "question": "What environmental recognition has the Garhwal Regimental Centre received?",
        "options": [
...
# --- Add this new function after all other functions but before main() ---
def show_data_structures_info(surface):
    """Display comprehensive information about data structures used in the module"""
    screen.blit(background_image, (0, 0))

    # Title
    title_text = title_font.render("Module 1: Foundation Data Structures", True, CYAN)
    screen.blit(title_text, (WIDTH // 2 - title_text.get_width() // 2, 50))

    # DSA Topics
    topics_text = big_font.render("DSA Topics: Arrays, Lists, Hash Tables", True, YELLOW)
    screen.blit(topics_text, (WIDTH // 2 - topics_text.get_width() // 2, 120))

    # Theme
    theme_text = font_medium.render("Theme: Personnel Database & Equipment Management", True, GREEN)
    screen.blit(theme_text, (WIDTH // 2 - theme_text.get_width() // 2, 170))

    # Where Used section
    where_used_title = font_medium.render("Where Used:", True, WHITE)
    screen.blit(where_used_title, (100, 220))

    where_used_points = [
        "Arrays: Question banks, score tracking, equipment inventory",
        "Lists: Dynamic user management, training records",
        "Hash Tables: Fast user lookup, equipment specifications, settings"
    ]

    for i, point in enumerate(where_used_points):
        text = font.render(point, True, WHITE)
        screen.blit(text, (120, 260 + i * 30))

    # Purpose section
    purpose_title = font_medium.render("Purpose:", True, WHITE)
    screen.blit(purpose_title, (100, 360))

    purpose_points = [
        "Manage trainee information efficiently",
        "Quick access to equipment data",
        "Store and retrieve training progress"
    ]

    for i, point in enumerate(purpose_points):
        text = font.render(point, True, WHITE)
        screen.blit(text, (120, 400 + i * 30))

    # Code examples section
    examples_title = font_medium.render("Implementation Examples:", True, WHITE)
    screen.blit(examples_title, (100, 500))

    example_texts = [
        "1. Database tables (users, modules, progress)",
        "2. Question bank array in quiz_game()",
        "3. User session hash table in get_logged_in_user_id()"
    ]

    for i, text in enumerate(example_texts):
        rendered = font.render(text, True, YELLOW)
        screen.blit(rendered, (120, 540 + i * 30))

    # Back button
    back_text = font.render("Press any key to return to menu", True, GREEN)
...
def pdf_reader():
    ...
    try:
        pdf_path = r"C:\Users\B.PURNA\PycharmProjects\DefenseShot\study_materials\module_1.pdf"  # Make sure this file exists
...
def practice_range():
    ...
    background = background_image  # Use the loaded image instead of creating a gradient
    ...
    while True:
        ...
        # Draw everything
        screen.blit(background_image, (0, 0))  # Instead of screen.blit(background, (0, 0))
...
def main():
    ...
    while True:
        if current_state == "menu":
            ...
            # Handle menu input
            # Handle menu input - UPDATED TO HANDLE NEW OPTION
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    pygame.quit()
                    sys.exit()
