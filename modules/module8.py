...
# DSA Implementations for Intelligence Analysis
class IntelligenceAnalyzer:
    def __init__(self):
        self.data = []

    def quick_sort_threats(self, threats):
        if len(threats) <= 1:
            return threats

        pivot = threats[len(threats) // 2]
        left = [t for t in threats if t.priority < pivot.priority]
        middle = [t for t in threats if t.priority == pivot.priority]
        right = [t for t in threats if t.priority > pivot.priority]

        return self.quick_sort_threats(left) + middle + self.quick_sort_threats(right)

    def merge_sort_chronological(self, events):
        if len(events) <= 1:
            return events

        mid = len(events) // 2
        left = self.merge_sort_chronological(events[:mid])
        right = self.merge_sort_chronological(events[mid:])

        return self.merge(left, right)

    def heap_sort_priority(self, missions):
        import heapq
        heap = [(-mission.priority, mission) for mission in missions]  # Negative for max heap
        heapq.heapify(heap)

        sorted_missions = []
        while heap:
            priority, mission = heapq.heappop(heap)
            sorted_missions.append(mission)

        return sorted_missions


def detect_threat_patterns(communication_logs):
    ...
    threat_patterns = ["attack", "infiltrate", "sabotage", "breach"]
    detected_threats = []

    for log in communication_logs:
        for pattern in threat_patterns:
            matches = kmp_search(log.content.lower(), pattern)
            if matches:
...
def open_module_9():
    ...
    possible_paths = [
        r"C:\Users\B.PURNA\PycharmProjects\DefenseShot\modules\module9.py",  # Original path
        os.path.join(os.path.dirname(__file__), "modules", "module9.py"),  # Relative to current file
        os.path.join(os.path.dirname(__file__), "module9.py"),  # Same directory
        "modules/module9.py",  # Relative path
        "module9.py"  # Current directory
    ]
...
questions = [
    {
        "question": "On what date was the Indian Air Force officially established as an auxiliary air force of the British India?",
        "options": ["A. 26 January 1950", "B. 8 October 1933", "C. 6 February 2004", "D. 8 October 1932"],
        "answer": "D",
        "difficulty": "Easy",
        "category": "History"
    },
    {
        "question": "What is the primary mission of the Indian Air Force as defined in the document?",
        "options": ["A. Conduct ground operations", "B. Secure maritime borders", "C. Provide disaster relief", "D. Secure Indian airspace and conduct aerial warfare"],
        "answer": "D",
        "difficulty": "Easy",
        "category": "Mission"
    },
    {
        "question": "Which aircraft was one of the first used by the Indian Air Force, as mentioned in the document?",
        "options": ["A. Sukhoi Su-30MKI", "B. Dassault Rafale", "C. MiG-29", "D. Westland Wapiti"],
        "answer": "D",
        "difficulty": "Medium",
        "category": "Historical Aircraft"
    },
    {
        "question": "During which conflict did the Indian Air Force fly over 16,000 sorties, as noted in the document?",
        "options": ["A. Sino-Indian War of 1962", "B. Kargil War of 1999", "C. Indo-Pakistani War of 1965", "D. Bangladesh Liberation War of 1971"],
        "answer": "D",
        "difficulty": "Medium",
        "category": "Operations"
    },
    {
        "question": "Which squadron, equipped with English Electric Canberras, supported the United Nations Operation in the Congo in 1960?",
        "options": ["A. No. 104 Firebirds", "B. No. 125 Gladiators", "C. No. 52 Sharks", "D. No. 5 Squadron"],
        "answer": "D",
        "difficulty": "Medium",
        "category": "International Operations"
    },
    {
        "question": "What was the name of the operation conducted by the Indian Air Force in Sri Lanka involving a supply drop, as described in the document?",
        "options": ["A. Operation Sindoor", "B. Operation Balakot", "C. Operation Kargil", "D. Operation Poomalai"],
        "answer": "D",
        "difficulty": "Hard",
        "category": "Operations"
    },
    {
        "question": "Which Indian Air Force officer was the first sportsperson and civilian without an aviation background to be awarded the honorary rank of group captain?",
        "options": ["A. Abhinandan Varthaman", "B. Hina Siddiqui", "C. Ajay Bhatt", "D. Sachin Tendulkar"],
        "answer": "D",
        "difficulty": "Hard",
        "category": "Honorary Ranks"
    },
    {
        "question": "What is the name of the Indian Air Force's special forces unit established on 6 February 2004?",
        "options": ["A. Surya Kiran", "B. Sarang", "C. Battleaxes", "D. Garud Commando Force"],
        "answer": "D",
        "difficulty": "Hard",
        "category": "Special Forces"
    },
    {
        "question": "Which indigenous fighter jet was the first to enter service with the Indian Air Force, as mentioned in the document?",
        "options": ["A. Dassault Mirage 2000", "B. Sukhoi Su-30MKI", "C. HAL Tejas", "D. HAL HF-24 Marut"],
        "answer": "D",
        "difficulty": "Hard",
        "category": "Indigenous Aircraft"
    },
    {
        "question": "What is the designation of the Indian Air Force's aerobatic display team that was conferred squadron status in 2006?",
        "options": ["A. No. 104 Firebirds", "B. No. 125 Gladiators", "C. Sarang", "D. 52 Squadron (The Sharks)"],
        "answer": "D",
        "difficulty": "Hard",
...
def draw_menu(surface):
    ...
    subtitle_text = big_font.render("Defense Training Module 8", True, WHITE)
    ...
    options = [
        ...
        "2. Take Quiz",
        "3. Practice Range",
        "4. Intelligence Analysis Tools",  # New option
        "5. Exit"
    ]
...
def pdf_reader():
    ...
    try:
        pdf_path = r"C:\Users\B.PURNA\PycharmProjects\DefenseShot\study_materials\module_8.pdf"  # Make sure this file exists
...
def intelligence_tools():
    """Show DSA implementations for intelligence analysis"""
    ...
    # Sample data
    threats = [
        Threat(1, 3, "Potential cyber attack"),
        Threat(2, 1, "Immediate physical threat"),
        Threat(3, 2, "Suspicious network activity")
    ]

    missions = [
        Mission(101, 1, "Critical infrastructure defense"),
        Mission(102, 3, "Routine patrol"),
        Mission(103, 2, "VIP protection detail")
    ]

    logs = [
        LogEntry(1001, "Monitor for any signs of attack in sector 7"),
        LogEntry(1002, "Unauthorized attempt to infiltrate mainframe"),
        LogEntry(1003, "All clear in designated areas")
    ]
    ...
    while True:
        ...
        # Title
        title = title_font.render("Intelligence Analysis Tools", True, YELLOW)
        ...
        # Quick Sort example
        sorted_threats = analyzer.quick_sort_threats(threats)
        sort_text = font_medium.render("Sorted Threats (Quick Sort by Priority):", True, GREEN)
        screen.blit(sort_text, (100, y_offset))
        y_offset += 40

        for i, threat in enumerate(sorted_threats):
            threat_text = font.render(f"{i + 1}. Priority {threat.priority}: {threat.description}", True, WHITE)
            screen.blit(threat_text, (120, y_offset))
            y_offset += 30

        y_offset += 20

        # Heap Sort example
        sorted_missions = analyzer.heap_sort_priority(missions)
        heap_text = font_medium.render("Sorted Missions (Heap Sort by Priority):", True, GREEN)
        screen.blit(heap_text, (100, y_offset))
        y_offset += 40

        for i, mission in enumerate(sorted_missions):
            mission_text = font.render(f"{i + 1}. Priority {mission.priority}: {mission.name}", True, WHITE)
            screen.blit(mission_text, (120, y_offset))
            y_offset += 30

        y_offset += 20

        # Pattern Matching example
        pattern_text = font_medium.render("Threat Pattern Detection (KMP Algorithm):", True, GREEN)
        screen.blit(pattern_text, (100, y_offset))
        y_offset += 40

        detected = detect_threat_patterns(logs)
        if detected:
            for result in detected:
                log_text = font.render(f"Found '{result['pattern']}' in log {result['log'].id}", True, RED)
                screen.blit(log_text, (120, y_offset))
                y_offset += 30
        else:
            no_threats = font.render("No threat patterns detected", True, WHITE)
            screen.blit(no_threats, (120, y_offset))
            y_offset += 30

        # Instructions
        instructions = font.render("Press ESC to return to menu", True, WHITE)
...
def main():
    ...
    while True:
        if current_state == "menu":
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    elif event.key == pygame.K_4:  # New option for intelligence tools
                        current_state = "intel_tools"
        ...
        elif current_state == "intel_tools":
            result = intelligence_tools()
            if result == "exit":
                pygame.quit()
                sys.exit()
            else:
