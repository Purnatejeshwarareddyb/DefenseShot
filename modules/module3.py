# Enhanced Module 3: PDF + Quiz with Progress Tracking and Next Module Button
# module3.py code with database integration and progress unlocking
...
def init_database():
    ...
    modules_data = [
        ...
        (3, "Strategic Planning & Decision Trees", "Hierarchical decision making and priority systems", 1, 80),
...
def open_module_4():
    """Open module_4.py file with improved path handling"""

    # Try multiple possible paths
    possible_paths = [
        r"C:\Users\B.PURNA\PycharmProjects\DefenseShot\modules\module4.py",  # Original path
        os.path.join(os.path.dirname(__file__), "modules", "module4.py"),  # Relative to current file
        os.path.join(os.path.dirname(__file__), "module4.py"),  # Same directory
        "modules/module4.py",  # Relative path
        "module4.py"  # Current directory
    ]
    ...
    if module2_path:
        try:
            ...
            print(f"❌ Error opening Module 4: {e}")
            return False
    else:
        print("❌ Module 4 file not found in any of these locations:")
        for path in possible_paths:
            print(f"   - {path}")

        # Show user-friendly error message
        show_error_message(pygame.display.get_surface(), "Module 4 file not found!", "Please check file path")
        return False
...
# --- Command Stack Classes ---
class Command:
    def __init__(self, description, execute_fn, undo_fn):
...
class CommandStack:
    ...
    def push(self, command):
        ...
        self.redo_stack = []  # Clear redo stack when new command is pushed
        command.execute()
    ...
    def get_history(self):
        return [cmd.description for cmd in self.stack]


# --- Tree Structure Classes for Module 3 ---
class TacticalDecisionTree:
    def __init__(self, decision):
        self.decision = decision
        self.left_outcome = None  # If condition A
        self.right_outcome = None  # If condition B
        self.success_rate = 0.0

    def add_outcome(self, outcome, is_left=True):
        if is_left:
            self.left_outcome = outcome
        else:
            self.right_outcome = outcome
...
class PerformanceTracker:
    ...
    def insert_score(self, user_id, score):
        new_node = {'user_id': user_id, 'score': score, 'left': None, 'right': None}
        if not self.root:
            self.root = new_node
        else:
    ...
    def find_user_rank(self, user_id):
        scores = []
        self._inorder_traversal(self.root, scores)
        for i, entry in enumerate(reversed(scores)):
            if entry['user_id'] == user_id:
                return i + 1  # 1-based ranking
        return -1

    def _inorder_traversal(self, node, scores):
        if node:
            self._inorder_traversal(node['left'], scores)
            scores.append(node)
            self._inorder_traversal(node['right'], scores)

    def find_user_score(self, user_id):
        """Helper method to find a user's current score"""
        return self._find_score_recursive(self.root, user_id)

    def _find_score_recursive(self, node, user_id):
        if not node:
            return 0
        if node['user_id'] == user_id:
            return node['score']
        left_score = self._find_score_recursive(node['left'], user_id)
        if left_score > 0:
            return left_score
        return self._find_score_recursive(node['right'], user_id)


class MissionPriorityQueue:
    def __init__(self):
        self.missions = []

    def add_mission(self, priority, mission):
        heapq.heappush(self.missions, (-priority, mission))  # Using negative for max heap

    def get_highest_priority(self):
        if self.missions:
            priority, mission = heapq.heappop(self.missions)
...
questions = [
    {
        "question": "What is the calibre of the INSAS rifle used by the Indian Army?",
        "options": ["A. 7.62mm", "B. 9mm", "C. 5.56mm", "D. 12.7mm"],
        "answer": "C",
        "difficulty": "Easy",
        "category": "Weapons"
    },
    {
        "question": "Which rifle was introduced to phase out the INSAS rifle due to its issues?",
        "options": ["A. SIG 716i", "B. FN SCAR L", "C. AK-203", "D. M4 carbine"],
        "answer": "C",
        "difficulty": "Easy",
        "category": "Weapons"
    },
    {
        "question": "Which of the following is a bullpup assault rifle used by Indian special forces?",
        "options": ["A. AK-203", "B. SIG 716i", "C. IMI Tavor TAR-21", "D. FN SCAR L"],
        "answer": "C",
        "difficulty": "Medium",
        "category": "Weapons"
    },
    {
        "question": "Which sniper rifle is being phased out by the Indian Army?",
        "options": ["A. IMI Galli 7.62 Sniper", "B. Heckler & Koch PSG1", "C. Dragunov SVD", "D. Vidhwansak"],
        "answer": "C",
        "difficulty": "Medium",
        "category": "Weapons"
    },
    {
        "question": "Which of the following is an Indian-made anti-material rifle?",
        "options": ["A. IMI Galli 7.62 Sniper", "B. Heckler & Koch PSG1", "C. Vidhwansak", "D. Dragunov SVD"],
        "answer": "C",
        "difficulty": "Medium",
        "category": "Weapons"
    },
    {
        "question": "Which of the following is a Russian multiple rocket launcher system used by the Indian Army?",
        "options": ["A. Pinaka MBRL", "B. K9 Vajra-T", "C. Smerch 9KS8 MBRL", "D. Dhanush"],
        "answer": "C",
        "difficulty": "Hard",
        "category": "Artillery"
    },
    {
        "question": "Which of the following is India's main battle tank developed by DRDO?",
        "options": ["A. T-90S Bhishma", "B. T-72 Ajeya", "C. Arjun", "D. BMP-2 Sarath"],
        "answer": "C",
        "difficulty": "Hard",
        "category": "Tanks"
    },
    {
        "question": "Which of the following is a supersonic cruise missile jointly developed by India and Russia?",
        "options": ["A. Agni-V", "B. Prahaar", "C. BrahMos", "D. Shaurya"],
        "answer": "C",
        "difficulty": "Hard",
        "category": "Missiles"
    },
    {
        "question": "Which of the following is a tactical maneuver where one unit moves while the other provides cover fire?",
        "options": ["A. Encirclement", "B. Frontal Assault", "C. Bounding Overwatch", "D. Hedgehog Defense"],
        "answer": "C",
        "difficulty": "Hard",
        "category": "Tactics"
    },
    {
        "question": "Which of the following is a defensive tactic that involves establishing a perimeter defense?",
        "options": ["A. Counterattack", "B. Fighting Withdrawal", "C. All-Round Defense", "D. Mutual Support"],
        "answer": "C",
        "difficulty": "Hard",
        "category": "Tactics"
    }
...
def draw_menu(surface):
    ...
    subtitle_text = big_font.render("Defense Training Module 3", True, WHITE)
...
def pdf_reader():
    ...
    try:
        pdf_path = r"C:\Users\B.PURNA\PycharmProjects\DefenseShot\study_materials\module_3.pdf"  # Make sure this file exists
...
def command_structure_demo():
    """Module 3 - Command Structure and Tree Structures demonstration"""
    command_stack = CommandStack()

    # Initialize data structures
    decision_tree = TacticalDecisionTree("Engage enemy?")
    decision_tree.add_outcome(TacticalDecisionTree("Flank maneuver"))
    decision_tree.add_outcome(TacticalDecisionTree("Direct assault"), is_left=False)
    decision_tree.left_outcome.success_rate = 0.85
    decision_tree.right_outcome.success_rate = 0.65

    performance_tracker = PerformanceTracker()
    performance_tracker.insert_score(get_logged_in_user_id(), 85)  # Current user
    performance_tracker.insert_score(102, 92)
    performance_tracker.insert_score(103, 78)

    mission_queue = MissionPriorityQueue()
    mission_queue.add_mission(3, "Secure perimeter")
    mission_queue.add_mission(1, "Evacuate civilians")
    mission_queue.add_mission(2, "Disarm explosives")

    # Sample commands for demonstration
    def deploy_troops():
        print("Troops deployed to forward position")
        # Update mission queue when troops deployed
        mission_queue.add_mission(2, "Support deployed troops")

    def recall_troops():
        print("Troops recalled to base")
        # Update decision tree when recalling
        decision_tree.right_outcome.success_rate = 0.7  # Adjusted strategy

    def activate_defense():
        print("Defense systems activated")
        # Update performance tracker
        performance_tracker.insert_score(get_logged_in_user_id(),
                                         performance_tracker.find_user_score(get_logged_in_user_id()) + 5)

    def deactivate_defense():
        print("Defense systems deactivated")

    commands = [
        Command("Deploy Scout Team", deploy_troops, recall_troops),
        Command("Activate Radar", activate_defense, deactivate_defense),
        Command("Establish Perimeter",
                lambda: [print("Perimeter established"), mission_queue.add_mission(1, "Reinforce perimeter")],
                lambda: print("Perimeter dismantled")),
        Command("Call Air Support",
                lambda: print("Air support inbound"),
                lambda: print("Air support recalled")),
        Command("Initiate Evac",
                lambda: [print("Evacuation begun"), mission_queue.add_mission(0, "Emergency evacuation")],
                lambda: print("Evacuation canceled"))
    ]

    current_command_idx = 0
    demo_complete = False
    show_structures = False  # Toggle for showing data structures
    ...
    while True:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_ESCAPE:
                    ...
                    command_stack.redo()
                elif event.key == pygame.K_n:  # Next command
                    if current_command_idx < len(commands):
                        command_stack.push(commands[current_command_idx])
                        current_command_idx += 1
                elif event.key == pygame.K_c:  # Complete demo
                    demo_complete = True
                elif event.key == pygame.K_s:  # Toggle structures
                    show_structures = not show_structures

        # Draw everything
        screen.blit(background_image, (0, 0))

        # Title
        title_text = title_font.render("Module 3: Command & Tree Structures", True, YELLOW)
        ...
        instructions = [
            ...
            "Press R to redo undone command",
            "Press S to toggle data structures",
            "Press C to complete demo",
        ...
        # Command history
        history_title = font_medium.render("Command History:", True, CYAN)
        screen.blit(history_title, (100, 350))

        history = command_stack.get_history()
        if not history:
            no_history = font.render("No commands executed yet", True, GRAY)
            screen.blit(no_history, (120, 400))
        else:
            for i, cmd in enumerate(history[-5:]):  # Show last 5 commands
                cmd_text = font.render(f"{i + 1}. {cmd}", True, GREEN if i == len(history) - 1 else WHITE)
                screen.blit(cmd_text, (120, 400 + i * 30))

        # Redo stack
        redo_title = font_medium.render("Undone Commands:", True, CYAN)
        screen.blit(redo_title, (WIDTH // 2 + 100, 350))

        if command_stack.redo_stack:
            for i, cmd in enumerate(command_stack.redo_stack[-5:]):
                cmd_text = font.render(f"{i + 1}. {cmd.description}", True, RED)
                screen.blit(cmd_text, (WIDTH // 2 + 120, 400 + i * 30))
        else:
            no_redo = font.render("No commands to redo", True, GRAY)
            screen.blit(no_redo, (WIDTH // 2 + 120, 400))

        # Data Structures Visualization
        if show_structures:
            # Decision Tree
            tree_title = font_medium.render("Decision Tree:", True, ORANGE)
            screen.blit(tree_title, (100, 500))

            tree_root = font.render(f"Root: {decision_tree.decision}", True, WHITE)
            screen.blit(tree_root, (120, 540))

            left_branch = font.render(
                f"← {decision_tree.left_outcome.decision} ({decision_tree.left_outcome.success_rate * 100}%)",
                True, GREEN)
            screen.blit(left_branch, (140, 570))

            right_branch = font.render(
                f"→ {decision_tree.right_outcome.decision} ({decision_tree.right_outcome.success_rate * 100}%)",
                True, RED)
            screen.blit(right_branch, (140, 600))

            # Performance Tracker
            perf_title = font_medium.render("Performance Ranking:", True, ORANGE)
            screen.blit(perf_title, (WIDTH // 2 + 100, 500))

            rank = performance_tracker.find_user_rank(get_logged_in_user_id())
            perf_text = font.render(f"Your rank: #{rank}", True, WHITE)
            screen.blit(perf_text, (WIDTH // 2 + 120, 540))

            # Mission Queue
            mission_title = font_medium.render("Mission Queue:", True, ORANGE)
            screen.blit(mission_title, (100, 650))

            next_mission = mission_queue.get_highest_priority()
            if next_mission:
                mission_text = font.render(f"1. {next_mission[1]} (Prio: {next_mission[0]})", True, YELLOW)
                screen.blit(mission_text, (120, 690))

                # Peek at next missions without removing
                temp_missions = mission_queue.missions.copy()
                if len(temp_missions) > 1:
                    heapq.heappop(temp_missions)
                    next2 = heapq.heappop(temp_missions)
                    mission_text2 = font.render(f"2. {next2[1]} (Prio: {-next2[0]})", True, WHITE)
                    screen.blit(mission_text2, (120, 720))
            else:
                no_missions = font.render("No missions in queue", True, GRAY)
                screen.blit(no_missions, (120, 690))

        # Completion message
        if demo_complete:
            complete_text = font_medium.render("Module 3 Complete! Press ESC to return", True, GREEN)
            pygame.draw.rect(screen, BLACK, (WIDTH // 2 - 200, HEIGHT - 100, 400, 50))
            pygame.draw.rect(screen, GREEN, (WIDTH // 2 - 200, HEIGHT - 100, 400, 50), 2)
            screen.blit(complete_text, (WIDTH // 2 - complete_text.get_width() // 2, HEIGHT - 85))

            # Update progress
            user_id = get_logged_in_user_id()
            update_progress(user_id, 3, 100)  # Full marks for completing demo
            unlock_next_module(user_id, 3)
...
def main():
    ...
    while True:
        ...
        elif current_state == "module3":
            result = command_structure_demo()
            if result == "exit":
                pygame.quit()
                sys.exit()
            else:
